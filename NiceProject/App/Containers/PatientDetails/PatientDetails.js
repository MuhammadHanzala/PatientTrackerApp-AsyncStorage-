...
function mapDispatchToProps(dispatch) {
    return {
        update: (newDetails, index, callback) => dispatch(editEntry(newDetails, index, callback)),
        delete: (index, callback) => dispatch(deleteEntry(index, callback))
    }
}
...
class PatientDetails extends Component {
    constructor(props) {
        super(props);
        const { patientName, diseases, medicationProvided, cost, } = this.props.navigation.state.params[1];
        this.state = {
            patient: this.props.navigation.state.params[1],
    ...
    update = () => {
        ...
        if (patientName === '' || diseases === '' || medicationProvided === '' || cost === null) {
            ...
            this.props.update(entryDetails, this.props.navigation.state.params[0], () => {
                this.setState({
                    editingMode: false,
                    patient: entryDetails,
                    patientName,
                    diseases,
                    medicationProvided,
                    cost,
            ...
            console.log(entryDetails, this.props.navigation.state.params[0])
        }
    }
    ...
    delete = () => {
        ...
        this.props.delete(this.props.navigation.state.params[0], async () => {
            navigate('HomePage');
        })
    }
                            ...
                            <View style={{
                                ...
                                <View style={{ flex: 1 }}>
                                    ...
                                    <View style={{ flexDirection: 'row', marginBottom: 15 }}>
                                        ...
                                        <Text style={{ fontSize: 18 }}>{(new Date(patient.patientId)).toLocaleString()}</Text>
                                    </View>
                                </View>
                            </View>
                    }
